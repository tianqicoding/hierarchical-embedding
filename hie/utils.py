...
def load_data(args):
	...
	print (len(graph.edges()), "removing all edges with zero weight")
...
def perform_walks(graph, features, args):
	...
	if not os.path.exists(walk_file):
		...
		feature_sim = None#make_feature_sim(features)
		...
		if features is not None and args.alpha>0:
			feature_sim=make_feature_sim(features)
			node2vec_graph.set_features(feature_sim)
