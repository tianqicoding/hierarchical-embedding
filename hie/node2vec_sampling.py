...
class Graph():
	...
	def preprocess_transition_probs(self):
		...
		with mp.pool.Pool(processes=None) as p:
			# node_divisor = len(p._pool) * 4
			# node_chunks = list(self.chunks(graph.nodes(), int(graph.order() / node_divisor)))
			# num_chunks = len(node_chunks)
			# print(num_chunks)
			alias_nodes = p.map(self.get_alias_node, graph.nodes())#, chunksize=num_chunks)
		alias_nodes = {node: alias_node for node, alias_node in alias_nodes}
		print(len(alias_nodes))
	...
	def set_features(self, feature_sim):
